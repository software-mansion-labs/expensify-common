...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
            ...
            },
            {
                ...
                rawInputReplacement: (match, extraLeadingUnderscores, textWithinUnderscores) => {
                    ...
                    return `${extraLeadingUnderscores}${this.splitByEmojis(textWithinUnderscores, '<em>', '</em>')}`;
                }
            },
...

            {
                ...
                rawInputReplacement: (match, g1) => g1.includes('</pre>') ? match : this.splitByEmojis(g1, '<strong>', '</strong>'),
            },
            {
                name: 'strikethrough',
                regex: /(?<!<[^>]*)\B~((?![\s~])[\s\S]*?[^\s~](?<!\s))~\B(?![^<]*>)(?![^<]*(<\/pre>|<\/code>|<\/a>))/g,
                replacement: (match, g1) => (g1.includes('</pre>') || this.containsNonPairTag(g1) ? match : `<del>${g1}</del>`),
                rawInputReplacement: (match, g1) => (g1.includes('</pre>') ? match : this.splitByEmojis(g1, '<del>', '</del>')),
            },
            {
                name: 'newline',
                regex: /\r?\n/g,
    ...
    splitByEmojis(text, startTag, closeTag) {
        // split by emoji tag <emoji>...</emoji>
        const splittedText = text.split(/(<emoji[^>]*>.*?<\/emoji>)/g);
        let result = '';

        splittedText.forEach((split) => {
            if (split.startsWith('<emoji')) {
                result += split;
                return;
            }

            // find last open tag and first close tag - we are embedding the new tag between them
            const endTagMatch = split.match(/<\/([a-z][a-z0-9-]*)\b[^>]*>|<\/([a-z][a-z0-9-]*)\s*>/);
            const startTagMatch = this.findLastRegexMatch(split, /<([a-z][a-z0-9-]*)\b[^>]*>|<([a-z][a-z0-9-]*)\s*>/g);
            
            const insertStartTagIndex = startTagMatch ? startTagMatch.index + startTagMatch[0].length : 0;
            const insertEndTagIndex = endTagMatch ? endTagMatch.index : split.length;

            const toAdd = `${split.substring(0, insertStartTagIndex)}${startTag}${split.substring(insertStartTagIndex, insertEndTagIndex)}${closeTag}${split.substring(insertEndTagIndex)}`;
            result += toAdd;
        });
        return result;
    }
